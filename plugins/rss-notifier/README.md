...
自动轮询发现新 item 时，会向订阅目标发送一条 `OutboundMessage`：文本中包含订阅源名、标题、发布时间、链接和正文摘要；如果配置允许且图片下载成功，会附带图片附件。`/rss_latest` 查询最近条目时也使用同一套单条 item 渲染逻辑，并按条目逐条发送，因此每条 latest 消息都会按正常通知规则携带自己的图片附件。
