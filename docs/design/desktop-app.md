...
| `/api/generate/text` | 通用一次性文本生成（当前唯一调用方是番茄钟动态文案）：服务端注入 workspace 人设前缀（system 基线 + AGENTS/SOUL/USER.md），客户端上传任务指令，可选预热 TTS 缓存 |
...
| `desktop.pomodoro.control` | Gateway -> Desktop | start/stop/toggle/status/configure 本地番茄钟；`workMinutes`/`breakMinutes`/`totalRounds`/提醒文案/`enabled`/`speakReminders`/`dynamicText`/`dynamicTextModel` 可选，configure 先落盘再生效。agent 工具 `desktop_pomodoro` 走此能力 |
...
- 番茄钟计时仍完全在 Desktop 本地运行，但已成为 `nahida.pomodoro` 的首个内置 Desktop runtime facet：独立 Gateway 插件拥有 agent 工具，`DesktopPluginHost` 拥有本地 service、`desktop.pomodoro.control` capability、start/stop/toggle action、设置面板和 `pet.overlay/countdown` surface。设置保存在 `desktopPluginSettings["nahida.pomodoro"]`，不随 pet runtime snapshot 分发；旧 `LocalDesktopConfig.pomodoro` 自动迁移。`pluginSurfaces` 随 `DesktopRuntimeSnapshot` 下发到 pet 窗口，剩余秒数由宿主按 `expiresAt` 每秒派生，快照不逐秒重发；桌宠 hidden/retreating 时隐藏。Gateway 插件 surface 通过 `desktop.surface.sync` 完整快照同步，本地 contribution 不会被远端快照清除。
- 插件 UI 四个槽位均由 Desktop 原生组件渲染：`desktop.home` 位于桌面助手运行页，`desktop.sidebar` 是主窗口全局可折叠侧栏，`pet.overlay` 是桌宠顶部轻量状态，`pet.drawer` 是桌宠交互模式下才能展开的只读抽屉。抽屉关闭或桌宠退出交互模式时不保留展开状态，避免透明窗口长期截获鼠标。
