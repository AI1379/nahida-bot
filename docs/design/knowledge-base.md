...
意图触发的增量是：让**未链接的访客**、或需要比默认 cascade 更宽的召回时，也能按意图扩张。
检测到这类意图时，把本轮可读 scope 集合扩张到**请求者本人**的 person/account scope。

**关键（§2.5）**：记忆是软上下文、不承载安全——访问控制不在记忆层，而在动作授权层。所以
这里的扩张**只影响召回的相关性**，没有任何 sensitivity / visibility 硬过滤要绕过（记忆侧
本就没有，见 person-identity-system.md §5.3）。访问语义（“本人”如何解析、scope cascade）
以其 §4 / §5.2 为准；本节只负责**触发判断**（该不该扩张可读 scope），不重复推导访问语义。

- 唯一的软约束是回复礼貌：召回进上下文 ≠ 该在群里复述私密来源——person-identity-system.md
  §10.2 的“私下召回、群里慎言”提示仍适用（UX 约束，非安全边界）。
- 意图判别器仍应保守：假阳性只是把不太相关的内容拉进上下文（回错话，无害），而非隐私泄露——
  因为记忆越权在 §2.5 下本就无害。
