...
class ChatAddress:
    ...
    def __post_init__(self) -> None:
        # TODO: Check if these validations will effect performance.
        if not self.channel:
    ...
    def parse(cls, value: str) -> ChatAddress:
        ...
        if is_valid_target_type(parts[1]):
            # Typed: milky:group:20001[:thread_id]
            return cls(
                channel=channel,
                target_type=parts[1],
                target_id=parts[2],
        ...
        # Second segment is NOT a known target type → legacy format.
        return cls(
            channel=channel,
            target_type=TARGET_TYPE_UNKNOWN,
            target_id=parts[1],
    ...
    def from_inbound(
        ...
        *,
        ...
        Prefer explicit ``chat_type``. Falls back to ``is_group`` heuristic.
        When neither provides a known type, returns ``unknown`` rather than
        guessing ``private``.
        """
        ...
        # Cannot determine type — do not guess private
...
class SessionKey:
    ...
    def parse(cls, value: str) -> SessionKey:
        ...
        Handles typed base keys, legacy base keys, and suffixed variants.
        """
        if not value:
        ...
        # Typed base: parts[1] is a canonical target type
        if is_valid_target_type(parts[1]):
            address = ChatAddress(
                channel=channel,
                target_type=parts[1],
        ...
        # Legacy with suffix: milky:10001:abcd1234 or milky:10001:cron:job_id
        target_id = parts[1]
        suffix = ":".join(parts[2:])
        return cls(
            address=ChatAddress(
